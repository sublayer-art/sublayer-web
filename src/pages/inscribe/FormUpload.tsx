...
import { FormItemContext } from "@/components/Form/FormItem";
import FormContext from "@/components/Form/context";
import { getIn } from "formik";
import Http from "@/tools/http";

const FormUpload: React.FC<UploadProps> = ({ onChange, ...resetProps }) => {
    ...
    if (file && onChange) {
      ...
      Http.post("/storage/upload", formData)
        .then((res) => {
          ...
          res.url = "https://sublayer.art" + res.url;
          setPreviewPath( res.url);
          onChange?.(res);
        })
        .finally(() => {
          setUploading(false);
      ...
      >
        ...
        {uploading ? (
          <Stack alignItems="center" justifyContent="center" height="100%">
            <CircularProgress />
            <Typography mt={1}>Uploading...</Typography>
          </Stack>
        ) : previewPath ? (
          <img
            src={previewPath}
